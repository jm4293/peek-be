...
export class StockService {
  ...
  async uploadStock(params: { file: Express.Multer.File; dataType: 'KOSPI' | 'KOSDAQ' }) {
    ...
    const category = await this.boardCategoryRepository.findByEnName(dataType);

    if (!category) {
      throw new BadRequestException(`${dataType} 카테고리가 존재하지 않습니다.`);
    }

    await this._saveStockCodes({ arr, dataType, boardCategoryId: category.id });
  }
  ...
  private async _saveStockCodes(params: { arr: any[]; dataType: 'KOSPI' | 'KOSDAQ'; boardCategoryId: number }) {
    const { arr, dataType, boardCategoryId } = params;
    ...
    if (dataType === 'KOSPI') {
      const entities = arr.map((row) => {
        ...
        entity.boardCategoryId = boardCategoryId;
